# TODO: count bounds exceeded (bim_error(...) == 10000)
# Which gradient function should I take vor the padding? should the params be different?
...
fit_bim <- function(observed_data, padding) {
  ...
  if (length(unique(observed_data[, 1])) == 1) {
    warning(
      'Confidence ratings for all trials are the same. Estimation of parameters Pexp and rho is inaccurate.'
    )
    w <- 1
  }
  ...
  if (length(unique_performance) == 1) {
    warning(
      'Performance for all trials is the same. Skipping fitting for this dataset.'
    )
    return(list(params = NA, logL = NA, w = 1))  # Return NA values for parameters and log likelihood
  }
