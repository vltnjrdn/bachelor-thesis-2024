...
folder_name <- sprintf("Results_Parameter_Recovery_%.d_%.d__%s", ntrial, nsimulation, format(Sys.time(), "%Y.%m.%d_%H.%M.%S"))
dir.create(folder_name, showWarnings = FALSE)
setwd(folder_name)

write.csv(corr.matrix.Pexp, paste0("corr_matrix_Pexp_", ntrial, ".csv"), row.names = TRUE)
write.csv(corr.matrix.Mconf, paste0("corr_matrix_Mconf_", ntrial, ".csv"), row.names = TRUE)
write.csv(corr.matrix.mu_m, paste0("corr_matrix_mu_m_", ntrial, ".csv"), row.names = TRUE)
write.csv(corr.matrix.rho, paste0("corr_matrix_rho_", ntrial, ".csv"), row.names = TRUE)
...
cat(
  ...
  paste("Pexp - Correlation:", result_Pexp$correlation, "Quality:", result_Pexp$quality, "\n"),
  paste("Mconf - Correlation:", result_Mconf$correlation, "Quality:", result_Mconf$quality, "\n"),
...
plots[[1]] <- plot_correlation(corr.matrix.Pexp, (expression(P[exp])), c(0.1, 0.9), c(0, 1), c(seq(0.1, 0.9, by = 0.1)), c(seq(0, 1, by = 0.2)))
plots[[2]] <- plot_correlation(corr.matrix.Mconf, (expression(M[conf])), c(0.1, 0.9), c(0, 1), c(seq(0.1, 0.9, by = 0.1)), c(seq(0, 1, by = 0.2)))
plots[[3]] <- plot_correlation(corr.matrix.mu_m, (bquote(mu[m])), c(-2, 2), c(-5, 5), c(seq(-2, 2, by = 0.5)), c(seq(-5, 5, by = 2.5)))
plots[[4]] <- plot_correlation(corr.matrix.rho, (bquote(rho)), c(-1, 1), c(-1, 1), c(seq(-1, 1, by = 0.5)), c(seq(-1, 1, by = 0.5)))
