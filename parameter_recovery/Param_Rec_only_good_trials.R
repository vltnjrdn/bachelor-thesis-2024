...
corr.matrix.mu_m <-
  data.frame(true = true_params[, 3], fitted = fit_params[, 3])
corr.matrix.rho <-
  data.frame(true = true_params[, 4], fitted = fit_params[, 4])
...
param_recovery <- function(true_params, fitted_params) {
  ...
  correlation <-
    cor(true_params, fitted_params, use = 'pairwise.complete.obs')
...
# extract results
result_mu_m <-
  param_recovery(corr.matrix.mu_m$true, corr.matrix.mu_m$fitted)
result_rho <-
  param_recovery(corr.matrix.rho$true, corr.matrix.rho$fitted)
...
folder_name <- sprintf(
  'Results_Parameter_Recovery_only_good_trials_%.d_%.d__%s',
  ntrial,
  nsimulation,
