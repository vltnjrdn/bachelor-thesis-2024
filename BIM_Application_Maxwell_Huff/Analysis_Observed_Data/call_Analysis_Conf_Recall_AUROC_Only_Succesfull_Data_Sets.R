...
# between conditions as well as paired t-tests to compare mean Recall with mean confidence within a condition
# for the observed data from Maxwell & Huff.
# If the data contains outliers, the analysis will be done with and without them.
# This function requires Analysis_Recall, Analysis_Confidence, Analysis_AUROC and to be loaded into the global environment.
...
setwd("../BIM_Application_Maxwell_Huff/Analysis_Observed_Data/")
...
for (i in seq_along(data_list)) {
  ...
  for (j in 1:nVpn) {
    # Eextract data for current person
...
# save results in txt
sink(file = "Analysis_Observed_Data_OSD.txt")
...
print("Results AUROC")
print(results_AUROC)
results_Confidence_vs_Recall <- Analysis_Confidence_vs_Recall(result_df[, c("ID", "Condition", "mean_Recall", "mean_Confidence")])
print("Results Confidence vs Recall")
print(results_Confidence_Recall)
sink(file = NULL)

print("Results have been saved to 'Analysis_Observed_Data_OSD.txt'")
